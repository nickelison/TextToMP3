...
resource "aws_security_group" "ecs_sg" {
  name        = "${local.app_name}-${var.ecs_sg_name}"
...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  family                   = "${local.app_name}-${var.ecs_service_name}"
  network_mode             = var.ecs_task_network_mode
  requires_compatibilities = ["EC2"]
  container_definitions    = <<DEFINITION
[
  {
    "name": "${local.app_name}-${var.django_container_name}",
...
resource "aws_lb" "main" {
  name                       = "${local.app_name}-${var.ecs_alb_name}"
  internal                   = false
...
resource "aws_lb_target_group" "django_ecs_tg" {
  name        = "${local.app_name}-${var.django_ecs_tg_name}"
  port        = "8000"
...
resource "aws_lb_listener" "alb_listener_port_443" {
  ...
  default_action {
    target_group_arn = aws_lb_target_group.django_ecs_tg.arn
    type             = "forward"
  }
}
...
resource "aws_ecs_cluster" "ecs_cluster" {
  name = "${local.app_name}-${var.ecs_cluster_name}"
}

# Create the ECS service.
resource "aws_ecs_service" "service" {
  name                 = "${local.app_name}-${var.ecs_service_name}"
  ...
  load_balancer {
    container_name   = "${local.app_name}-${var.django_ecs_service_container_name}"
    container_port   = var.django_ecs_service_container_port
    target_group_arn = aws_lb_target_group.django_ecs_tg.arn
  }
}
...
resource "aws_autoscaling_group" "ecs_asg" {
  name                 = "${local.app_name}-${var.ecs_asg_name}"
...
resource "aws_autoscaling_policy" "ecs_infra_scale_out_policy" {
  name                   = "${local.app_name}_${var.ecs_asg_policy_name}"
...
resource "aws_appautoscaling_target" "ecs_service_scaling_target" {
  ...
  resource_id        = "service/${aws_ecs_cluster.ecs_cluster.name}/${local.app_name}-${var.ecs_service_name}"
...
# Create a task definition for database upgrades.
resource "aws_ecs_task_definition" "db_upgrade" {
  family                   = "${local.app_name}-${var.ecs_service_name}-db-upgrade"
  network_mode             = var.ecs_task_network_mode
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        awslogs-group         = var.ecs_migrations_log_group_name
        awslogs-region        = var.region
    ...
    "secrets" : [
      {
        name      = "POSTGRES_USER"
        valueFrom = "arn:aws:secretsmanager:${local.region}:${local.account_id}:secret:${var.env_vars_secret_id}"
      },
      {
        name      = "POSTGRES_HOST"
        valueFrom = "arn:aws:secretsmanager:${local.region}:${local.account_id}:secret:${var.env_vars_secret_id}"
