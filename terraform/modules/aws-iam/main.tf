    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:secretsmanager:${local.region}:${local.account_id}:secret:${var.env_vars_secret_id}",
          "arn:aws:secretsmanager:${local.region}:${local.account_id}:secret:${var.cf_public_key_secret_id}"
        ]
      }
    ]
  })
...
# Attach the custom CloudWatch logs policy to the ECS task execution IAM role.
